...
class App {
    ...
    // Properties of the object, as private class field (#).
    #map;
    #mapZoomLevel = 14;
    #mapEvent;
    #workout = [];
    ...
    constructor() {
        ...
        // Functions called as soon as the app is launched
        this._getPosition();
        this._getlocalStorage();

        // bind.(this) point to the app object
    ...
    _getPosition() {
        // 'if' ensure there's no error if old browsers don't support goelocation
        if (navigator.geolocation) {
            // Function takes two callback functions: 1st one is call on succes when browser get coordinates of users, 2nd one is call on error when trying to get coordinates
    ...
    _newWorkout(event) {
        ...
        // Function used to validate each inputs in workout form are numbers
        const validNumberInput = function (...inputs) {
            // Loop over the ...inputs array and check if each number is finite or not. every( ) method return true only if all elements in the array are finite.
        ...
        // Render workout on the side part of UI (left-bar outside the map)
    ...
    // Function used to display popup over workout pins in map
